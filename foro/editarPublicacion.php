...
if($titulo && idUsuarioLogado() && $fecha && $contenido && $tipo && $juego){
    ...
    Publicacion::actualiza($publicacion);

    if($publicacion ){
        Utils::redirige(Utils::buildUrl('/foro.php', ['exito' => $id]));
    ...
    Utils::redirige(Utils::buildUrl('/foro.php?accion=editarPublicacion', ['error' => 'datosInvalidos']));
}